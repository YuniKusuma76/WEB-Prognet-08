...
<head>
    ...
    <title>Family Card - Edited Kartu Keluarga</title>
    ...
    <div class="main-content">
        ...
        <main>
          <div class="card">
            ...
            <div class="card-body">
              <form name="formKk" action="/kk/{{ $kk->id }}" method="post" onsubmit="return validateForm()">
                @csrf
                @method('PUT')
                  <div class="form-group">
                    <label>No. KK</label>
                    <input type="text" name="nokk" id="nokk" placeholder="Nomor Kartu Keluarga" class="form-control" value="{{ $kk->nokk }}">
                  </div>
                  <div class="form-group">
                    <label>Status Aktif</label>
                    <select name="statusaktif" id="statusaktif" class="form-control">
                        <option value="">-- Pilih --</option>
                        <option value="1" @if ($kk->statusaktif=='1'){{ "selected" }} @endif>Aktif</option>
                        <option value="0" @if ($kk->statusaktif=='0'){{ "selected" }} @endif>Tidak Aktif</option>
                    </select>
                  </div>
                  <div>
          ...
          <script>
            function validateForm() {
              // Validasi NO. KK
              if (document.forms["formKk"]["nokk"].value == "") {
                  alert("Anda belum memasukkan Nomor Kartu Keluarga Anda");
                  document.forms["formKk"]["nokk"].focus();
                  return false;
              }
              // Validasi Status Aktif
              if (document.forms["formKk"]["statusaktif"].selectedIndex < 1) {
              alert("Anda belum memilih Status Aktif Kartu Keluarga Anda");
              document.forms["formKk"]["statusaktif"].focus();
              return false;
              }
            }
        </script>
